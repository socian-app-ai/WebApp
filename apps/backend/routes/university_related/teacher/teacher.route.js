...
console.log(id, ":in review commetn")
    try {
        ...
        const teacher = await Teacher.findById(id).populate({
            ...
            populate: {
                path: 'userId',
                select: '_id name username personalEmail universityEmail profile universityEmailVerified personalEmailVerified'
            }
        });
        ...
        const populatedRatings = await Promise.all(teacher.ratingsByStudents.map(async (review) => {
            let userIdData;
            if (review.userId) {
                userIdData = {
            ...
            const userVote = await UserReviewTeacherVote.findOne({ reviewId: review._id, userId: review.userId?._id });
...
router.post('/reviews/comments/vote',async (req, res) => {
    ...
    try {
        ...
        const existingVote = await UserReviewTeacherVote.findOne({ reviewId, userId }).session(session);

        if (existingVote) {
            if (existingVote.voteType === voteType) {

                review[voteType === 'upvote' ? 'upvoteCount' : 'downvoteCount']--;
                await UserReviewTeacherVote.deleteOne({ _id: existingVote._id }).session(session);
            } else {
            ...
            review[voteType === 'upvote' ? 'upvoteCount' : 'downvoteCount']++;
            await UserReviewTeacherVote.create([{ reviewId, userId, voteType }], { session });
        }
